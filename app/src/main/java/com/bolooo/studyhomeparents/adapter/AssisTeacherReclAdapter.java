...
public class AssisTeacherReclAdapter extends BaseRecycleViewAdapter<JSONObject> {
    ArrayList<String> mGuides;
    TeacherAssisImagesAdapter imageAdapter;
    private float score;

    public AssisTeacherReclAdapter(Context context) {
        super(context);
    }
    ...
    public class Holder extends BaseRecycleViewHolder<JSONObject> {
        ...
        public void loadData(JSONObject data, int position) {
            if (data.length() > 0) {
                if (data.optString("HeadPhoto").length() == 0){
                    Glide.with(mContext).load(data.optString("WeChatHeadPhoto")).error(R.drawable.noavatar).into(civUserImg);
                }else{//加载headphoto
                    Glide.with(mContext).load(Constants.imageUrl+data.optString("HeadPhoto")+"?w=300&h=300").error(R.drawable.noavatar).into(civUserImg);
                }
                ...
                score = (float)data.optDouble("Score");
                assisStar.setRating((float) data.optDouble("Score"));
                assisScore.setText(score+"");
                ...
                if (commentTags.length() > 0) {
                    ...
                    CouserTypeAdapter adapter = new CouserTypeAdapter(mContext, "assistTag");
                ...
                if (commentImgs.length() > 0){
                    ...
                    imageAdapter= new TeacherAssisImagesAdapter(mContext);
