...
public class NewNearLessonAdapter extends NBaseAdapter<LookCourseEnity.AreasEntity> {

    private int checkItemPosition = 0;

    public NewNearLessonAdapter(Context context) {
        super(context);
    }
    ...
    @Override
    public BaseViewHolder<LookCourseEnity.AreasEntity> getNewHolder(View view) {
        return new MyCityViewHolder(view);
    }

    public class MyCityViewHolder extends BaseViewHolder<LookCourseEnity.AreasEntity> {
        @Bind(R.id.text)
        TextView mText;
        ...
        @Override
        public void loadData(LookCourseEnity.AreasEntity data, int position) {
            if (data == null) return;
