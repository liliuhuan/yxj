...
public class CourseOrderListAdapter extends BaseRecycleViewAdapter<CourseOderEntity> {
    ...
    @Override
    public BaseRecycleViewHolder<CourseOderEntity> getNewHolder(View view) {
        return new CourseOrderViewHolder(view);
    }

    public class CourseOrderViewHolder extends BaseRecycleViewHolder<CourseOderEntity> {
        ...
        @Override
        public void loadData(CourseOderEntity data, int position) {
            if (data == null) return;
            orderNumber.setText("订单号： "+data.getOrderNum());
            glideUtils.loadFileImageRound(data.getHeadPhoto(),teacherImage);
            teacherName.setText(data.getTeacherName());
            teacherCourse.setText("《 "+data.getCourseName()+" 》"+ data.FrequencyName+" 班");
            ...
            switch (orderStatus){
                ...
                case 6://已取消
                    ...
                    break;
                case 5://报名失败
                    str = "状态：<font color='#999999'>审核未通过</font>";
                    break;
                case 9://家长未上课，已过期
            ...
            bundle.putString("frequencyId",data.FrequencyId);
            itemView.setOnClickListener(V-> IntentUtils.startNewIntentBundle(mContext,bundle, LessonsOrderDetailActivity.class));
        }
    }
}
