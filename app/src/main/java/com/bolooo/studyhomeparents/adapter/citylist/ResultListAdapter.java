...
public class ResultListAdapter extends BaseAdapter {
    ...
    public ResultListAdapter(Context mContext, List<CityListBean> mCities) {
        this.mCities = mCities;
        this.mContext = mContext;
    }

    public void changeData(List<CityListBean> list){
        if (mCities == null){
            mCities = list;
        }else{
            mCities.clear();
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        holder.name.setText(mCities.get(position).ProvinceName+" - "+mCities.get(position).CityName);
        return view;
    }
